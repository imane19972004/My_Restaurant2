Feature: Manage Restaurant Opening Hours
  ...
  So that the schedule is always up to date
        ...
        Scenario: Successfully add multiple time slots for the same day
            Given the restaurant is open on "WEDNESDAY" from "11:30" to "14:00"
             When the owner adds new opening hours for "WEDNESDAY" from "18:30" to "22:00"
             Then the restaurant should have 2 time slots for "WEDNESDAY"
              And the restaurant schedule should contain hours for "WEDNESDAY" from "11:30" to "14:00"
              And the restaurant schedule should contain hours for "WEDNESDAY" from "18:30" to "22:00"
